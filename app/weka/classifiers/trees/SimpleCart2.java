 ...
 */
public class SimpleCart2 extends RandomizableClassifier implements
	...
	/** Successor nodes. */
	protected SimpleCart2[] m_Successors;
	 ...
	 */
	public SimpleCart2[] getSuccessors() {
		return m_Successors;
	}
	...
	protected void makeTree(Instances data, int totalInstances,
		...
		if (totalWeight < 2 * minNumObj || giniGains[attIndex] == 0
			...
			m_isLeaf = false;
			m_Successors = new SimpleCart2[2];
			for (int i = 0; i < 2; i++) {
				m_Successors[i] = new SimpleCart2();
	...
	public void prune(double alpha) throws Exception {
		...
		while (prune) {
			...
			SimpleCart2 nodeToPrune = nodeToPrune(nodeList);
	 ...
	 */
	protected SimpleCart2 nodeToPrune(Vector nodeList) {
		if (nodeList.size() == 0)
			return null;
		if (nodeList.size() == 1)
			return (SimpleCart2) nodeList.elementAt(0);
		SimpleCart2 returnNode = (SimpleCart2) nodeList.elementAt(0);
		double baseAlpha = returnNode.m_Alpha;
		for (int i = 1; i < nodeList.size(); i++) {
			SimpleCart2 node = (SimpleCart2) nodeList.elementAt(i);
	...
	public static void main(String[] args) {
		runClassifier(new SimpleCart2(), args);
	}
}
